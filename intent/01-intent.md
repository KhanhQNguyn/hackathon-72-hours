...
- What problem are we solving?
  ...
  ~~General "in-app task completion" for any workplace app~~ — **narrowed further to a concrete target.** We are building an AI layer, on top of/alongside VoiceOver, that helps a blind user complete a **product/food order on Shopee/ShopeeFood** — the first concrete instance of a pattern that applies equally to other button-dense, multi-step apps such as ride-hailing (Grab, Be, XanhSM), though only Shopee/ShopeeFood is built and demoed in this 72h hackathon — without having to linearly swipe through every unrelated element on screen. Founder's own tested pain points using VoiceOver on Shopee to order a product:
...
- **Target app/flow:** **Shopee only**, real app (no mocked/lookalike UI — confirmed, this matters for credibility with industry-partner judges), scoped to **2–3 pre-tested products/search queries** rather than general search. ShopeeFood or other flows are roadmap-only, not built. **Product vision vs. build, distinguished:** the underlying pattern (AI-filtered, checkpoint-confirmed navigation of a complex third-party app) is designed to generalize to other apps in the same category — e.g. ride-hailing apps like Grab, Be, XanhSM — but only Shopee/ShopeeFood is actually built, tested, and demoed for this hackathon; state this distinction explicitly in the deck rather than letting it blur.
