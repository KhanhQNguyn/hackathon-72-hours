...
The repo is both a **planning-artifact chain** (Markdown under `docs/`) **and an in-progress Flutter app** (`lib/`, `assets/js/`, `test/`, Flutter package name `job_access_assist`). Work is tracked as 47 milestones in `docs/intent/milestones/` (`00-index.md` is the dependency map; each `milestoneNN-*.md` is self-contained). Check the index and `git log` to see which are done — as of now the code for milestones 1–44 exists, but that is not the same as working end to end: `docs/intent/audit-2026-09-22.md` is the current, honest status (what is verified, what needs a device or an API key, and the known gaps — notably no navigation from the listing page to the application form). Still needing a person with a phone: 25 (real-target manual test), 43 (TalkBack + Accessibility Scanner; only Flutter's automated guideline checks in `test/ui/accessibility_test.dart` are done), 45 (repeated real runs), and the on-device parts of 35/44. 46 (demo video) and 47 (slide deck) are not code. The OpenAI prompts (36–40) are only tested against a scripted HTTP client — they have never run against the real model. **Android only:** the app shows an explicit message on any other platform (`sqflite` and the WebView `WebMessageListener` have no Windows/Linux implementation); do not add desktop support. **Read the milestone file before implementing or changing a piece** — it has preconditions, Definition of Done, and flagged open decisions.
...
- Mic UX: `VoiceCommandGate` is the only caller of `speech.listen()` and emits the single "mic open" signal (`onListeningChanged`, opt-in spoken cues via `playCues`); the controller republishes it as `isListening`, which `VoiceTriggerButton` reads (not the FSM state — the mic is also open during in-flow confirmations). After an error the user can say "retry" (`_retryByVoice`). Error text always comes from `narration_lookup.dart` (`FlowFailure` carries localized messages through the FSM).
